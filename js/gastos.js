	...
	$.ajax({
	  ...
	  success: function(r){
		...
		if(r.bool){
			...
			document.getElementById("totalGas").value = r.total;
			$(".totalgasto").val(r.total);
			...
			get_items_cot(r.id_evento);
			checarTotal('cotizaciones',r.id_evento);
			$(".guardar").hide();
			$(".modificar").show();
...
function eliminar_art(elemento){
	...
	if(id_item!=0){
		$.ajax({
			url:'scripts/quita_art_gasto.php',
			cache:false,
			type:'POST',
			data:{
				'id_item':id_item
...
function guardar_art(elemento){
	...
	if(id_cotizacion!=""){
		...
		//id_articulo = document.getElementById(elemento).value;
		//id_articulo = document.getElementById(elemento).value;
		//alert(id_articulo);
