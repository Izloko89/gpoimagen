...
try{
	...
	foreach($res->fetchAll(PDO::FETCH_ASSOC) as $i=>$v){
		...
		$aidi=$bd->query("SELECT id_usuario FROM usuarios WHERE usuario LIKE '$term';");
	$aidi = $aidi->fetchAll(PDO::FETCH_ASSOC);
	$res=$bd->query("DESCRIBE usuarios_contacto;");
	foreach($res->fetchAll(PDO::FETCH_ASSOC) as $a=>$c){
		$campos[$a]=$c["Field"];
	}
		$res=$bd->query("SELECT * FROM usuarios_contacto where id_usuario = " . $aidi[0]["id_usuario"]);
	foreach($res->fetchAll(PDO::FETCH_ASSOC) as $i=>$v){
		foreach($campos as $campo){
			$r[$i][$campo]=$v[$campo];
		}
