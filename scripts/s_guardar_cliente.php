...
if($cd == 0){
	...
	$sql="INSERT INTO `clientes_contacto`(`id`, `id_empresa`, `id_cliente`, `clave`, `direccion`, `colonia`, `ciudad`, `estado`, `cp`, `telefono`, `celular`, `email`, `contacto`) VALUES
								($clave, 1,$clave,$clave,'$direccion','$colonia', '$ciudad', '$estado', '$cp', '$telefono', '$celular', '$email', '$contacto');"; 	
	...
	$sql="UPDATE `clientes_fiscal` SET `id`=$clave, `id_empresa`=1, `id_cliente`=$clave, `rfc`='$rfc', `razon`='$razon', 
		...
		`estado`='$estado_fiscal', `cp`='$cp_fiscal' , `contacto`='$contacto'
		where id=$clave;"; 	
