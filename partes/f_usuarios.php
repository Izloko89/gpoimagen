...
function agregar_usr()
{
	...
	if(document.getElementById("fichacontrol").checked)
	{
		fichacontrol = 1;
	}
...
<form id="f_usuarios_permisos" class="formularios">
  <h3 class="titulo_form">PERMISOS</h3>
	...
	<div class="formularios" style="border:0;">
      <h3 class="titulo_form"><input type="checkbox" id="fichacontrol" name="fichacontrol" value="1" /> - Ficha de control</h3>
    </div>
</form>
...
<h3 class="titulo_form">Listado de usuarios registrados</h3>
    ...
    <?php if(count($usuarios)>0){foreach($usuarios as $art=>$d){
		...
		echo '<td><input type="button" value="eliminar" onclick="eliminar('.$d["id_usuario"].')"/></td>';
		echo '</tr>';
	}//foreach
	}//if end ?>
    </table>
...
function usuario(){
	...
	$.ajax({
	  ...
	  success: function(r){
		...
		document.getElementById("id_usuario").value = r[0].id_usuario;
		//alert(r[0].clave);
	...
	$.ajax({
	  ...
	  success: function(r){
		  ...
		  if(r){
			...
			if(r.gastos == 1)
			{
				document.getElementById("gastos").checked = true;
			}
			if(r.fichacontrol == 1)
			{
				document.getElementById("fichacontrol").checked = true;
			}
